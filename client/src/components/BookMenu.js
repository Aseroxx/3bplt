...
const getVisualEffectsStyles = (banner) => {
  ...
  if (
    banner.shadow_color &&
    (banner.shadow_blur > 0 || banner.shadow_x !== 0 || banner.shadow_y !== 0)
  ) {
...
const LockedPageElement = ({
  ...
  useEffect(() => {
    ...
    if (
      banner.is_locked &&
      banner.page_number !== null &&
      banner.page_number !== undefined
    ) {
      const updatePosition = () => {
        const pageContainer = document.querySelector(
          `.book-page-container[data-page-index="${banner.page_number - 1}"]`
        );
        const pageElement = pageContainer
          ? pageContainer.querySelector(".page")
          : null;
        if (pageElement) {
...
const BookMenu = ({
  ...
  selectedTextElement,
  onTextElementClick,
  onTextElementMouseDown,
  onTextElementContextMenu,
  pages: pagesProp,
  banners: bannersProp,
  textStyles: textStylesProp,
  siteTexts: siteTextsProp,
}) => {
  ...
  const isDesignMode =
    designMode ||
    (location.search.includes("design=true") && user?.role === "admin");
  ...
  const editOriginalTextsMode =
    isDesignMode && pagesMode && currentSelectedTool === "pages";
  // Textareas can only be edited in "edit pages" mode (not in "select" mode)
  const editTextareasMode =
    isDesignMode && pagesMode && currentSelectedTool === "pages";
  // Other items added by admin (text, image) can be edited with both 'select' and 'pages' tools
  const editItemsMode =
    isDesignMode &&
    (currentSelectedTool === "select" ||
      currentSelectedTool === "pages" ||
      pagesMode);
  const activeSelectedTextElement =
    selectedTextElement || designModeState?.selectedTextElement;
  const activeOnTextElementClick =
    onTextElementClick || designModeState?.onTextElementClick;
  const activeOnTextElementMouseDown =
    onTextElementMouseDown || designModeState?.onTextElementMouseDown;
  const activeOnTextElementContextMenu =
    onTextElementContextMenu || designModeState?.onTextElementContextMenu;
  ...
  const loadCustomFonts = async () => {
    try {
      ...
      fonts.forEach((font) => {
        ...
        const fontFace = new FontFace(font.font_family, `url(${encodedPath})`);
        fontFace
          .load()
          .then((loadedFont) => {
  ...
  useEffect(() => {
    const fixPages12 = async () => {
      try {
        ...
        await Promise.all([
          axios.put(
            `/api/admin/page-text-styles/1/system-info`,
          ...
          axios.put(
            `/api/admin/page-text-styles/2/system-info`,
            { position_x: null, position_y: null },
            {
              headers: {
                Authorization: `Bearer ${token}`,
  ...
  const getTextStyle = (pageNumber, elementType) => {
    const style = textStyles.find(
      (s) => s.page_number === pageNumber && s.element_type === elementType
    );
    return (
      style || {
  ...
  const nextPage = () => {
    ...
    if (currentSlide < totalSlides - 1) {
      setCurrentPage((currentSlide + 1) * 2);
    } else {
      setShowApplication(true);
    }
  };
  ...
  return (
    <div
      ...
      style={isDesignMode ? { overflow: "visible" } : {}}
    >
      <div className="book-header">
        <div className="terminal-header">
          <div className="terminal-buttons"></div>
          <span className="terminal-title">
            {siteTexts.terminal_title || "PROJECT_DOCUMENTATION.exe"}
          </span>
        </div>
        <div className="user-info">
          <span>
            {siteTexts.user_label || "USER:"} {user?.username}
          </span>
          {user?.role === "admin" && (
            <Link to="/admin" className="admin-link">
              {siteTexts.admin_panel_link || "ADMIN PANEL"}
            </Link>
          )}
          <button onClick={logout} className="logout-btn">
            {siteTexts.logout_button || "LOGOUT"}
          </button>
        </div>
      </div>
                  ...
                  <div className="page-content">
                    <div className="page-header">
                      {(() => {
                        ...
                        const pageNumberStyle = getTextStyle(
                          pageNumber,
                          "page-number"
                        );
                        const titleStyle = getTextStyle(pageNumber, "title");
                        const isSelected =
                          activeSelectedTextElement &&
                          activeSelectedTextElement.pageNumber === pageNumber;
                        return (
                          <>
                            <div
                              ...
                              onContextMenu={(e) => {
                                if (
                                  editOriginalTextsMode &&
                                  activeOnTextElementContextMenu
                                ) {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  activeOnTextElementContextMenu(
                              ...
                              style={{
                                ...
                                position:
                                  ...
                                  activeSelectedTextElement?.pageNumber ===
                                    pageNumber &&
                                  activeSelectedTextElement?.elementType ===
                                    "page-number"
                                    ? "fixed"
                                    : "relative", // Always use relative for page-number to prevent disappearing
                                left:
                                  editOriginalTextsMode &&
                                  isDraggingTextElement &&
                                  activeSelectedTextElement?.pageNumber ===
                                  ...
                                  activeSelectedTextElement?.elementType ===
                                    "page-number"
                                    ? `${textElementDragPosition?.x || 0}px`
                                    : "auto",
                                top:
                                  editOriginalTextsMode &&
                                  isDraggingTextElement &&
                                  ...
                                  activeSelectedTextElement?.elementType ===
                                    "page-number"
                                    ? `${textElementDragPosition?.y || 0}px`
                                    : "auto",
                                zIndex:
                                  editOriginalTextsMode &&
                                  isDraggingTextElement &&
                                  ...
                                  activeSelectedTextElement?.elementType ===
                                    ...
                                    ? 10006
                                    : "auto",
                                cursor: editOriginalTextsMode
                                  ? "move"
                                  : "default",
                                userSelect: editOriginalTextsMode
                                  ? "none"
                                  : "auto",
                              }}
                            >
                              ...
                              style={{
                                ...
                                position:
                                  ...
                                  activeSelectedTextElement?.elementType ===
                                    "title"
                                    ? "fixed"
                                    : "relative", // Always use relative for title to prevent disappearing
                                left:
                                  editOriginalTextsMode &&
                                  isDraggingTextElement &&
                                  activeSelectedTextElement?.pageNumber ===
                    ...
                    >
                      {(() => {
                        ...
                        const pageElements = banners.filter(
                          (banner) =>
                            banner.page_number === pageNumber &&
                            banner.is_active &&
                            (!isMobile || banner.page_number !== null)
                        );
                              ...
                              style={{
                                ...
                                position:
                                  ...
                                  activeSelectedTextElement?.elementType ===
                                    "content"
                                    ? "fixed"
                                    : contentStyle.position_x !== null &&
                                      contentStyle.position_y !== null
                                    ? "absolute"
                                    : "relative",
                                left:
                                  editOriginalTextsMode &&
                            ...
                            {pageElements
                              ...
                              .map((banner) => {
                                if (banner.type === "textarea") {
                                  ...
                                  const isEditableInPagesMode =
                                    editItemsMode &&
                                    selectedTextElement &&
                                    selectedTextElement.pageNumber ===
                                      pageNumber &&
                                    selectedTextElement.elementType ===
                                      "textarea" &&
                                    ...
                                    <textarea
                                      ...
                                      onContextMenu={(e) => {
                                        ...
                                        if (
                                          ...
                                          e.stopPropagation();
                                          activeOnTextElementContextMenu &&
                                            activeOnTextElementContextMenu(e, {
                                              pageNumber,
                                      ...
                                      style={{
                                        ...
                                        left:
                                          banner.position_x === null ||
                                          banner.position_x === undefined ||
                                          banner.position_x === 0 ||
                                          (banner.is_locked &&
                                            banner.position_x === 20)
                                            ? positionType === "absolute"
                                              ? "50%"
                                              : "calc(50vw - 185px)"
                                            : `${
                                                (banner.position_x || 0) - 40
                                              }px`, // Convert from page coordinates to page-body coordinates
                                        top:
                                          banner.position_y === null ||
                                          banner.position_y === undefined ||
                                          banner.position_y === 0 ||
                                          (banner.is_locked &&
                                            banner.position_y === 20)
                                            ? positionType === "absolute"
                                              ? "50%"
                                              : "calc(50vh - 225px)"
                                            : `${
                                                (banner.position_y || 0) - 40
                                              }px`, // Convert from page coordinates to page-body coordinates
                                        width: `${pageWidth}px`,
                                        ...
                                        fontSize: `${banner.font_size || 16}px`,
                                        fontWeight:
                                          banner.font_weight || "normal",
                                        fontStyle:
                                          banner.font_style || "normal",
                                        color: banner.color || "#ff00ff",
                                        backgroundColor: "rgba(0, 0, 0, 0.5)",
                                        border: isEditableInPagesMode
                                          ? "2px dashed #ff00ff"
                                          : "1px solid #ff00ff",
                                        padding: "10px",
                                        resize:
                                          ...
                                          user?.role !== "admin"
                                            ? "none"
                                            : "both", // Disable resize if locked or not admin
                                        zIndex: banner.is_locked
                                          ? (banner.z_index || 1) + 10000
                                          : (banner.z_index || 1) +
                                            (isPageLeft ? 0 : 100), // Right page elements have higher z-index
                                        pointerEvents:
                                          isDesignMode &&
                                          currentSelectedTool === "select" &&
                                          !editTextareasMode
                                            ? "none"
                                            : "auto", // Disable clicks in select mode
                                        cursor:
                                          banner.is_locked ||
                                          user?.role !== "admin" ||
                                          (isDesignMode &&
                                      ...
                                      onChange={async (e) => {
                                        ...
                                        if (
                                          ...
                                          window.__textareaDebounceTimers[
                                            ...
                                            if (user?.role !== "admin") {
                                              console.warn(
                                                "Unauthorized: User is not admin, reverting changes"
                                              );
                                              // Revert immediately
                                              setBanners((prevBanners) =>
                                                prevBanners.map((b) =>
                                            ...
                                            try {
                                              ...
                                              console.log(
                                                "Textarea content saved successfully"
                                              );
                                              ...
                                              console.error(
                                                "Error updating textarea content:",
                                                error
                                              );
                                  ...
                                  const activeHandleElementClick =
                                    designModeState?.handleElementClick;
                                  const activeHandleMouseDown =
                                    designModeState?.handleMouseDown;
                                  const activeHandleElementContextMenu =
                                    designModeState?.handleElementContextMenu;
                                  const isElementSelected =
                                    designModeState?.selectedElement ===
                                    banner.id;
                                  const isElementDragging =
                                    designModeState?.isDragging &&
                                    designModeState?.selectedElement ===
                                      banner.id;
                                  const currentSelectedTool =
                                    designModeState?.selectedTool || "select";
                                  ...
                                  const elementLeftInPage = isElementDragging
                                    ? currentElement.position_x || 0
                                    : banner.position_x || 0;
                                  const elementTopInPage = isElementDragging
                                    ? currentElement.position_y || 0
                                    : banner.position_y || 0;
                                      ...
                                      <div
                                        ...
                                        onClick={(e) => {
                                          ...
                                          const designModeState =
                                            window.__designModeState;
                                          const blockClick =
                                            designModeState?.getBlockClick?.();
                                          const justFinishedDragging =
                                            designModeState?.getJustFinishedDragging?.();
                                          const draggedElementId =
                                            designModeState?.getDraggedElementId?.();
                                          const hasMoved =
                                            designModeState?.getHasMoved?.();
                                        ...
                                        onContextMenu={(e) => {
                                          if (
                                            canEdit &&
                                            activeHandleElementContextMenu
                                          ) {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            activeHandleElementContextMenu(
                    ...
                    <div className="page-footer">
                      {(() => {
                        const pageNumber = index + 1;
                        const systemInfoStyle = getTextStyle(
                          pageNumber,
                          "system-info"
                        );
                        const isSelected =
                        ...
                        return (
                            ...
                            style={{
                              ...
                              position:
                                ...
                                activeSelectedTextElement?.elementType ===
                                  ...
                                  ? "relative"
                                  : systemInfoStyle.position_x !== null &&
                                    systemInfoStyle.position_y !== null
                                  ? "absolute"
                                  : "relative",
                              left:
                                editOriginalTextsMode &&
                          ...
                          >
                            ...
                            {">"}{" "}
                            {siteTexts.system_info_prefix ||
                              "SYSTEM: ONLINE | STATUS: ACTIVE | TIME:"}{" "}
          ...
          <div className="page-indicator">
            {Array.from({ length: totalSlides }).map((_, index) => (
              <div
                ...
                className={`dot ${
                  index ===
                  (isMobile ? currentPage : Math.floor(currentPage / 2))
                    ? "active"
                    : ""
                }`}
                onClick={() =>
                  isMobile ? setCurrentPage(index) : setCurrentPage(index * 2)
                }
              ></div>
            ))}
          </div>
      ...
      {!isDesignMode &&
        !isMobile && // <<==== BLOQUE LES ITEMS GLOBAUX SUR MOBILE
        banners &&
        banners.length > 0 &&
        banners
          .filter((banner) => {
            // Show global elements (page_number is null)
            if (banner.page_number === null || banner.page_number === undefined)
              return true;
          ...
          .map((banner) => {
            ...
            const fontStyle =
              banner.font_family && banner.font_family !== "JetBrains Mono"
                ? {
            ...
            if (
              ...
              const pageContainer = document.querySelector(
                `.book-page-container[data-page-index="${
                  banner.page_number - 1
                }"]`
              );
              const pageElement = pageContainer
                ? pageContainer.querySelector(".page")
